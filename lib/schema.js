...
// - idx_tasks_status: Common filters by status alone (e.g., queue length queries)
// - idx_tasks_type: Filters by task type
// - idx_tasks_chat: Filters by chat_id
// - idx_tasks_created: Sorted queries by creation time
// - idx_tasks_type_status: Composite for frequent (type, status) filters used in poll() loop
//   Example: WHERE type='work' AND status='backlog' - used to select next task to process
// - idx_tasks_chat_status: Composite for (chat_id, status) combinations
//   Example: WHERE chat_id=X AND status='inbox' - used for chat-specific filtering
...
const MIGRATIONS = [
  ...
  { name: "index_tasks_type_status", sql: "CREATE INDEX IF NOT EXISTS idx_tasks_type_status ON tasks(type, status)" },
  { name: "index_tasks_chat_status", sql: "CREATE INDEX IF NOT EXISTS idx_tasks_chat_status ON tasks(chat_id, status)" },
]
