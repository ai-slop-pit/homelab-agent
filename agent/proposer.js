...
function getTopicId(proposalType) {
  const config = loadConfig();
  return config.topics?.['system-proposals'];
}
...
async function proposeSkill(skillName, description, why, estimatedValue) {
  ...
  const topicId = getTopicId('skill');
  const sendOpts = {
    parse_mode: 'Markdown',
    reply_markup: {
      inline_keyboard: [
  ...
  await telegram.sendMessage(GROUP_ID, message, sendOpts);
